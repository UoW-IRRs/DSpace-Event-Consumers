	...
	public void consume(Context context, Event event) throws Exception {
		...
		if (StringUtils.isNotBlank(embargoField)) {
			...
			if (values != null && values.length > 0 && values[0] != null && StringUtils.isNotBlank(values[0].value)) {
				...
				String formattedDate = SimpleDateFormat.getDateInstance(DateFormat.MEDIUM).format(embargoDate.toDate());
				message.addArgument(I18nUtil.getMessage("lconz-extra.notify-author.embargo.message.text-preceding-date")
						+ formattedDate
						+ I18nUtil.getMessage("lconz-extra.notify-author.embargo.message.text-following-date"));
			}
		}
